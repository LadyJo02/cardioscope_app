...
class _ReportGeneratedPageState extends State<ReportGeneratedPage> {
  ...
  Future<void> _saveReportToDatabase() async {
    ...
    final report = {
      'patientName': nameToSave,
      'filePath': widget.filePath,
      'recordedDate': widget.recordedDate.toIso8601String(),
      'classification': widget.classification, // **USE ACTUAL RESULT**
      'confidence': widget.confidence,        // **USE ACTUAL RESULT**
    };
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Center(child: Text('Analysis Complete!', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold, color: Colors.green[800]))),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Patient Details', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                const Divider(height: 20),
                _buildDetailRow("Name:", widget.patientName),
                _buildDetailRow("File Location:", widget.filePath, isSelectable: true),
                _buildDetailRow("Recorded:", DateFormat('MMMM d, yyyy HH:mm').format(widget.recordedDate)),
              ]),
            ),
          ),
          const SizedBox(height: 16),
          Card(
            ...
            child: Padding(
              padding: const EdgeInsets.all(16.0),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Phonocardiogram (PCG)', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                const Divider(height: 20),
                SizedBox(
                  height: 150,
                  child: FutureBuilder<List<FlSpot>>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return LineChart(LineChartData(
                        ...
                        lineBarsData: [LineChartBarData(spots: snapshot.data!, isCurved: false, color: const Color(0xFFC31C42), barWidth: 1, dotData: const FlDotData(show: false))],
                        minY: -1, maxY: 1, lineTouchData: const LineTouchData(enabled: false),
                      ));
                    },
                  ),
                ),
          ...
          Card(
            ...
            child: Padding(
              padding: const EdgeInsets.all(16.0),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('AI Analysis', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                const Divider(height: 20),
  ...
  Widget _buildDetailRow(String label, String value, {bool isSelectable = false}) {
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: isSelectable ? SelectableText(value, textAlign: TextAlign.end) : Text(value, textAlign: TextAlign.end)),
      ]),
    );
  }
}